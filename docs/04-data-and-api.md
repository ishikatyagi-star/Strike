...
**executions** — `id` uuid PK (**the end-to-end idempotency key**) · `mandate_id` FK **UNIQUE** (single-fire lock #2) · `trigger_snapshot_id` FK→price_snapshots · `quote_total_cents` int · `prava_session_id` text nullable (Line P marker — under `PRAVA_MODE=mandate` this holds the mandate-charge `transactionId`) · `checkout_submitted_at` nullable (Line C marker) · `store_order_id` nullable · `outcome` CHECK IN ('fulfilled','aborted_rearmed','failed','revoked') nullable · `failure_reason` text nullable · `created_at`,`updated_at`.
...
| `POST /api/mandates/:id/sign` | session | `{assertion}` → `{mandate(signed)}` — runs Doc 2 §3.3; idempotent via nonce consumption (replays hit `NONCE_CONSUMED`) | `NONCE_EXPIRED`, `NONCE_CONSUMED` (409), `HASH_MISMATCH`, `BAD_SIGNATURE`, `ORIGIN_MISMATCH` (401) |
| `POST /api/mandates/:id/arm` | session | → `{approval_url}` — arm phase 1: creates the backing Prava one-time mandate (Doc 2 §7); mandate stays `signed` until approved | `NOT_SIGNED` (409), `PRAVA_ERROR` (502) |
| `POST /api/mandates/:id/confirm-arm` | session | → `{status:'armed', prava_mandate_id}` once the Prava mandate is active, else `{pending:true}`; links it, `signed`→`armed` + first snapshot | `NOT_SIGNED` (409), `PRAVA_ERROR` (502) |
